...
public class Phase1StandardReceiptPostingTests
{
    ...
    private static ReceiptInventoryPostingService Service(AppDbContext db)
        => new(db, Engine(db), new FakeCapabilities(true),
            valuation: new InventoryValuationService(db), standardCost: new StandardCostResolver(db, new StandardCostRollupService(db, new SystemClock(), StubCapabilitySnapshotProvider.Off)));
