...
- **Large uploads need TWO attributes, not one.** `[DisableRequestSizeLimit]` only lifts Kestrel's cap; the multipart reader has its own 128 MB default that kills the upload partway through with "Multipart body length limit exceeded". Any endpoint accepting a genuinely large `IFormFile` (database archives, drawing sets, bulk imports) also needs `[RequestFormLimits(MultipartBodyLengthLimit = …)]`. There is no global `FormOptions` override configured. Reference: `AdminDatabaseController.Import`.
- **Admin → Database Transfer** (`AdminDatabaseController` + `forge.api/Services/DatabaseTransferService.cs`, Admin role) dumps/imports all application data as a zip whose layout matches the forge-db CLI's `dump`/`import`, so archives are interchangeable in both directions. Import truncates and reloads the selected tables under suspended FK triggers (**superuser connection required**), then re-validates every FK and reports orphans. Excludes `hangfire`/`forge_db`/`__EFMigrationsHistory`, mirroring the pg-schema-diff reconcile exclusions.
...
6. **Codified via capability gating.** The boundary is enforced through the capability system; the only declared mutex is `CAP-ACCT-EXTERNAL ⊥ CAP-ACCT-BUILTIN`. **`CAP-ACCT-FULLGL` is a real, dependency-gated toggle — default OFF ("dark"), depends on `CAP-ACCT-BUILTIN`, mutex with `CAP-ACCT-EXTERNAL`** — backed by the substantially-built native GL. It is **not** an "aspirational placeholder": nothing force-blocks it, and the 403 when it's disabled is the standard capability-disabled response (`CapabilityGateBehavior` / `CapabilityDisabledException`), not a bespoke "not yet available" message. Enabling it is gated on posted opening balances (§7A conversion). **That gate is wired and shipped** (`765a8a4d`, 2026-07-07, in `v1.0.0-beta.2`): `ToggleCapabilityHandler` evaluates `IGlCapabilityGate` for every active book before flipping FULLGL on and returns 409 `capability-gl-opening-balances` when any book lacks a posted `JournalSource.Conversion` opening journal; installs with no Book pass trivially; disabling is ungated. Opening balances load via `POST /api/v1/accounting/conversion/opening-journal` with `GET .../conversion/tie-out` for the go-live trial-balance check (both `Controller` role, `AccountingConversionController`). **No UI covers that conversion workflow yet — it is API-only.** Authoritative, current descriptions live in `CapabilityCatalog.cs` (`CAP-ACCT-EXTERNAL` / `-BUILTIN` / `-FULLGL`) and `CapabilityCatalogRelations.cs`. See the **Capability Gating** section below for the mechanism.
