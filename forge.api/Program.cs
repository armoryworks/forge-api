...
try
{
    ...
    builder.Services.AddDbContext<AppDbContext>((sp, options) =>
        options
            ...
            // Suppress EF Core 9's "pending model changes" startup-time
            // validation. The existing migration history was generated by a
            // slightly different snake_case/FK naming pass than current EF
            // produces from the entity classes (cosmetic — single vs double
            // underscore between dependent + principal table). The migration
            // history is the source of truth for table/column/FK names; this
            // toggle stops MigrateAsync from blocking on a non-functional
            // diff. Revisit if/when migrations are regenerated from scratch
            // (forge-db DbUp project will replace this entirely).
            .ConfigureWarnings(w =>
