...
try
{
    ...
    builder.Services.AddDbContext<AppDbContext>((sp, options) =>
        options
            ...
            // Accounting GL Phase 0: posted-ledger immutability (§2, §4, §5.2).
            // OnConfiguring guards against double-add when DI supplies it here.
            // GlChangeBroadcastInterceptor pushes "accountingChanged" so the GL screens auto-refresh.
            .AddInterceptors(
                sp.GetRequiredService<Forge.Data.Interceptors.LedgerImmutabilityInterceptor>(),
                sp.GetRequiredService<Forge.Api.Features.Accounting.GlChangeBroadcastInterceptor>())
            // Suppress EF Core's "pending model changes" startup validation.
            // Schema is now owned by the declarative forge-db project (applied via
            // SchemaBootstrapper), not EF migrations — and forge-db's snake_case/FK
            // naming differs cosmetically from what EF would emit from the entity
            // classes (single vs double underscore between dependent + principal
            // table). forge-db is the source of truth for table/column/FK names;
            // this toggle stops EF from flagging that non-functional model-vs-schema
            // diff. The forge-db schema-drift check keeps the EF model and forge-db
            // aligned in CI.
            .ConfigureWarnings(w =>
    ...
    // Accounting GL foundation (Phase 0, acct_* schema). CAP-ACCT-FULLGL is
    // OFF/unwired — these services are registered but reached only via
    // IPostingEngine + read/config interfaces, so the code stays dark until a
    // command site calls the engine behind the capability gate.
    builder.Services.AddScoped<IAccountDeterminationResolver, Forge.Api.Features.Accounting.AccountDeterminationResolver>();
    ...
    // GL-change broadcaster: singleton (keeps EF's options/service-provider caching intact); per-context
    // dirty state lives in a ConditionalWeakTable inside the interceptor. Pushes "accountingChanged" so the
    // dark accounting screens auto-refresh on any GL write.
    builder.Services.AddSingleton<Forge.Api.Features.Accounting.GlChangeBroadcastInterceptor>();
    ...
    // ai-fleet-orchestration D-3 — live-fact provider (hybrid RAG freshness).
    ...
    using (var scope = app.Services.CreateScope())
    {
        ...
        {
            // Schema is owned by the declarative forge-db project. The canonical desired state
            // (forge.data/Schema/forge-schema.sql, assembled from forge-db's schema/ tree) is applied
            // by SchemaBootstrapper: the full schema on a fresh DB, a no-op when it already exists. EF
            // Core no longer runs migrations — the squash machinery (reconciler/verifier) was retired
            // with this cutover (2026-06-17); all schema changes now flow through forge-db.
            var forceRecreate = builder.Configuration.GetValue<bool>("RECREATE_DB");
            Log.Information("[DB-LIFECYCLE] Config: RECREATE_DB={RecreateDb}", forceRecreate);

            if (forceRecreate)
            {
                // Manual escape hatch — set RECREATE_DB=true to force a wipe (dev use only).
                Log.Warning(
                    "[DB-LIFECYCLE] ⚠ RECREATE_DB=true — DELETING the entire database and recreating from " +
                    "the forge-db schema. This destroys ALL data. If unintentional, check your .env file.");
                await db.Database.EnsureDeletedAsync();
                Log.Warning("[DB-LIFECYCLE] Database deleted; recreating from the forge-db declarative schema.");
            }
            ...
            Log.Information(
                schemaCreated
                    ...
                    : "[DB-LIFECYCLE] Existing schema detected — no schema changes applied (forge-db owns schema).");
