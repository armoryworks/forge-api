...
public class PageSpecificTraining : TrainingContentBase
{
    ...
    public override async Task SeedAsync()
    {
        ...
        await GetOrCreateModule(new TrainingModule
        {
            ...
            ContentJson = """{"body":"## Shipping Carriers\n\nThis page is the one home for your shipping carriers, and the only place you enter carrier sign-in details. Each row is a carrier — an integrated one such as UPS, FedEx, USPS, or DHL, or a custom shipper you set up yourself.\n\n### Key actions\n\n- **New Carrier** opens a window where you set the name, code, connection type (Manual or Api), how tracking updates arrive (Manual, Poll, or Webhook), and whether a scan is required to ship.\n- **Credentials** (the key icon) opens a form for the carrier's sign-in details — client ID, secret, an optional account number, and whether you're testing or live. The secret is stored securely and never shown back, so you re-enter it each time you edit.\n- **Test** (the signal icon) checks the live connection to the carrier and tells you whether it worked. It stays disabled until sign-in details are saved.\n\n### Manual vs Api carriers\n\nThe **connection type** you choose changes everything else on the row. A **Manual** carrier is just a label — your shippers type a tracking number by hand, so there are no sign-in details, no Test button, and no service ID. An **Api** carrier connects to the live carrier system, so it shows the service ID field when you create it, adds the key and signal icons to its row, and shows a yellow warning until sign-in details are saved. Pick Manual when you only need to record a tracking number. Pick Api when you want live rates, printed labels, and automatic tracking on the Shipments page.\n\n### How tracking updates arrive\n\nThis controls how a shipment's tracking status refreshes after it ships. **Manual** means someone updates the status by hand. **Poll** means Forge checks the carrier for the latest tracking events every so often. **Webhook** means the carrier sends status changes to Forge as they happen — the most up-to-date option, but only for carriers that support it. Manual is the safe default for a Manual carrier.\n\n### There is no row edit or deactivate\n\nThe table is kept simple on purpose: a carrier row has no edit or deactivate button. The only per-row controls are Credentials and Test, and those appear on Api carriers only. So plan the name, code, and type before you save.\n\n### Reading the table\n\nColumns show the name, code, connection type, whether a scan is required, a sign-in status label (green when set, a warning when an Api carrier has none), and whether the carrier is active. Carriers you set up here power the rate comparison, label creation, and tracking on the Shipments page — get the sign-in details right and Test passing before you rely on a carrier for real shipments.","sections":[]}"""
        });
        ...
        await GetOrCreateModule(new TrainingModule
        {
            ...
            ContentJson = """{"body":"## Automations — Event Failures\n\nMany Forge features do work quietly in the background — for example, reacting after a job or order is created. This page shows admins the background tasks that failed, so a hiccup never slips by unnoticed.\n\n### Reading the table\n\nEach row is a failed task. It shows what triggered it, what it was trying to do, a short error message (hover for the full text), its status by color (red for failed, orange for retrying, green for resolved), how many times it's been retried, when it first failed, and when it was last retried. Click a row to open a panel with the full error and the related details.\n\n### Key actions\n\n- The **status filter** narrows to All, Failed, Retrying, or Resolved, and **Refresh** reloads the list.\n- **Retry** (on a failed row) runs the task again, after you confirm.\n- **Resolve** (on a failed or retrying row) marks the failure as handled without retrying — useful once you've fixed the cause or decided a retry isn't needed.\n\n### How it fits\n\nThese controls are a troubleshooting tool for admins. A pile-up of failures points to a problem with an outside service or something inside Forge that couldn't finish its work.","sections":[]}"""
        });
        ...
        await GetOrCreateModule(new TrainingModule
        {
            ...
            ContentJson = """{"appRoute":"/admin/carriers","startButtonLabel":"Tour Carriers","steps":[{"element":"[data-testid='carrier-new-btn']","popover":{"title":"New Carrier","description":"Click here to add a carrier. You'll set a name, an optional code, the connection type (Manual or Api), how tracking updates arrive (Manual, Poll, or Webhook), and whether a scan is required to ship. Choose Api for live rates, labels, and tracking. Choose Manual when you only type a tracking number by hand.","side":"bottom"}},{"element":"app-data-table","popover":{"title":"The Carriers Table","description":"Every carrier is a row here. You'll see the name, code, connection type, whether a scan is required, its sign-in status, and whether it's active. An Api carrier with no sign-in details shows a yellow warning — your cue to open Credentials next. A row has no edit or deactivate button; the only per-row controls are Credentials and Test, and they appear on Api carriers only.","side":"top"}},{"element":"[data-testid='carrier-credentials-btn']","popover":{"title":"Enter Credentials","description":"On an Api carrier, the key icon opens the sign-in form: client ID, secret, an optional account number, and whether you're testing or live. The secret is stored securely and never shown back, so you re-enter it each time you edit. Save it to turn the sign-in status green.","side":"left"}},{"element":"[data-testid='carrier-test-btn']","popover":{"title":"Test the Connection","description":"The signal icon checks the live connection to the carrier and tells you whether it worked. It stays disabled until sign-in details are saved. Get this passing in test mode before you rely on the carrier for real shipments.","side":"left"}}]}"""
        });
        ...
        await GetOrCreateModule(new TrainingModule
        {
            ...
            Summary = "Reference for every field, label, and action on the Carriers admin page, including Manual vs Api and the three tracking-update options.",
            ...
            ContentJson = """{"title":"Carriers Field Reference","groups":[{"heading":"New Carrier Dialog","items":[{"label":"Name (required)","value":"The carrier's name, up to 100 characters (e.g. 'UPS', 'FedEx', 'Local Courier')."},{"label":"Code","value":"An optional short code, up to 50 characters, used to spot the carrier in lists and on documents."},{"label":"Connection Type (required)","value":"Manual or Api. Manual is a label only — no sign-in details, no test. Api connects to the live carrier and adds the service ID field plus the Credentials and Test buttons. Starts as Manual."},{"label":"Tracking Updates (required)","value":"Manual, Poll, or Webhook — how tracking status refreshes after a shipment leaves. Starts as Manual."},{"label":"Service ID","value":"Shown only for Api carriers. An optional ID that links this carrier to the shipping connection behind it. Up to 50 characters."},{"label":"Requires Scan to Ship","value":"On/off switch (on by default). When on, a package must be scanned before it can be marked shipped for this carrier."}]},{"heading":"Credentials Dialog (Api carriers only)","items":[{"label":"Client ID (required)","value":"The carrier's client or account ID, up to 200 characters. Not secret — it's shown filled in when you reopen the form."},{"label":"Secret (required)","value":"The carrier's secret key. It's stored securely and never shown back, so you re-enter it each time you edit."},{"label":"Account Number","value":"An optional carrier account number, up to 50 characters."},{"label":"Environment (required)","value":"Test mode or live. Use test mode to check the connection without touching real shipments; switch to live when you're ready."}]},{"heading":"Connection Type — what changes","items":[{"label":"Manual","value":"No sign-in details, no Test button, no service ID field. Your shippers type the tracking number by hand. Use when you don't need live rates or labels."},{"label":"Api","value":"Adds the service ID field when you create it, plus the key (Credentials) and signal (Test) icons in the row. Powers live rate comparison, label creation, and automatic tracking on the Shipments page."}]},{"heading":"Tracking Updates","items":[{"label":"Manual","value":"Someone updates the shipment's tracking status by hand. The safe default for a Manual carrier."},{"label":"Poll","value":"Forge checks the carrier for the latest tracking events every so often and updates the status."},{"label":"Webhook","value":"The carrier sends status changes to Forge as they happen — the most up-to-date option, available only for carriers that support it."}]},{"heading":"Table Columns","items":[{"label":"Carrier / Code","value":"Name and optional short code. Sort by either."},{"label":"Connection","value":"Manual or Api."},{"label":"Scan","value":"A scanner icon when a scan is required to ship, otherwise a dash."},{"label":"Credentials","value":"A green 'Configured' label when sign-in details exist; a yellow 'Not configured' warning when an Api carrier has none; a dash for Manual carriers."},{"label":"Active","value":"Shows whether the carrier is active or inactive."},{"label":"Actions","value":"The key (Credentials) and signal (Test) icons — Api carriers only. Test is disabled until sign-in details exist. A row has no edit or deactivate button."}]},{"heading":"Actions","items":[{"label":"New Carrier","value":"Opens the create window."},{"label":"Credentials (key icon)","value":"Opens the sign-in form for an Api carrier."},{"label":"Test (signal icon)","value":"Checks the live connection to the carrier and tells you whether it worked. Disabled until sign-in details exist."}]}]}"""
        });
        ...
        await GetOrCreateModule(new TrainingModule
        {
            ...
            ContentJson = """{"passingScore":80,"questionsPerQuiz":6,"shuffleOptions":true,"showExplanationsAfterSubmit":true,"questions":[{"id":"ca1","text":"You add a carrier and set its connection type to Manual. Which controls will you NOT see for that carrier?","options":[{"id":"a","text":"The Name and Code fields"},{"id":"b","text":"The Credentials (key) and Test (signal) buttons, and the service ID field","isCorrect":true},{"id":"c","text":"Whether the carrier is active in the table"},{"id":"d","text":"The tracking-updates setting"}],"explanation":"A Manual carrier is just a label — your shippers type the tracking number by hand. So it has no sign-in details, no Test button, and no service ID field. Only Api carriers show those, because they connect to the live carrier."},{"id":"ca2","text":"After saving an Api carrier with valid sign-in details, you reopen the Credentials form. What do you notice about the Secret field?","options":[{"id":"a","text":"It shows the saved secret so you can confirm it's correct"},{"id":"b","text":"It's empty — the secret is stored securely and never shown back, so you must re-enter it to save again","isCorrect":true},{"id":"c","text":"It's locked and can't be changed"},{"id":"d","text":"It shows the first and last four characters as a masked preview"}],"explanation":"The secret is never shown back. It's stored securely, so the form fills in the client ID and environment but leaves the secret blank. You re-enter the secret each time you edit the sign-in details."},{"id":"ca3","text":"The Test (signal) button is greyed out on a carrier row. Why?","options":[{"id":"a","text":"The carrier is set to Manual, or it's an Api carrier with no sign-in details saved yet","isCorrect":true},{"id":"b","text":"Your role can't test carriers"},{"id":"c","text":"The Shipments feature is turned off"},{"id":"d","text":"The carrier has already been tested once and can't be tested again"}],"explanation":"Test only appears on Api carriers, and it stays disabled until sign-in details exist. If you don't see it at all, the carrier is Manual; if it's there but greyed out, save the sign-in details first. The test checks a live connection to the carrier."},{"id":"ca4","text":"You want a carrier's tracking status to update the moment the carrier reports a scan, without Forge having to keep asking. Which tracking-updates option does this?","options":[{"id":"a","text":"Manual"},{"id":"b","text":"Poll"},{"id":"c","text":"Webhook","isCorrect":true},{"id":"d","text":"Auto"}],"explanation":"Webhook means the carrier sends status changes to Forge as they happen — the most up-to-date option. Poll means Forge checks the carrier for updates every so often. Manual means someone updates the status by hand. Webhook works only for carriers that support it."},{"id":"ca5","text":"You need to rename a carrier and change how its tracking updates arrive. What does the Carriers table let you do?","options":[{"id":"a","text":"Click the row's edit pencil to change any field"},{"id":"b","text":"There's no edit or deactivate button — the only per-row controls are Credentials and Test (Api carriers only)","isCorrect":true},{"id":"c","text":"Double-click the cell to edit it in place"},{"id":"d","text":"Drag the row to a 'Pending changes' area"}],"explanation":"The Carriers table is kept simple on purpose. A row shows only Credentials and Test, and those appear on Api carriers. There's no in-place edit or deactivate, so plan the name, code, and type before you save."},{"id":"ca6","text":"Before relying on a new Api carrier for real shipments, what's the recommended order?","options":[{"id":"a","text":"Save the carrier in live mode and ship a real package to confirm"},{"id":"b","text":"Create the carrier, save the sign-in details in test mode, run Test until it passes, then switch to live","isCorrect":true},{"id":"c","text":"Create the carrier and mark it active right away — no testing needed"},{"id":"d","text":"Email the carrier to confirm the account before setting anything up in Forge"}],"explanation":"Create the carrier, enter the sign-in details in test mode, and run Test until it passes. Once the connection works in test mode, switch to live. A passing test is your sign that the connection works before real shipments depend on it."}]}"""
        });
        ...
        await GetOrCreateModule(new TrainingModule
        {
            ...
            ContentJson = """{"body":"## Capability system\n\nEdge cases and admin notes beyond the browse-and-switch basics.\n\n- **History and reasons.** Every change — turning a feature on or off, editing its settings, or applying a preset or discovery result — can carry an optional reason that's saved with it. Each feature has its own history, and there's a full history page for all of them. Every entry records the before and after, who did it (or 'system'), what happened, when, and the reason when one was given.\n- **Two admins at once.** If someone else changes a feature while you have it open, saving your change is blocked so you don't overwrite them without knowing. Reload the row and try again.\n- **Dependencies and conflicts.** Turning on a feature that's missing something it needs, turning off one that others rely on, or turning on two features that conflict is refused with a clear message. When you turn several on or off at once, it's all-or-nothing — if any one breaks a rule, the whole batch is rejected.\n- **Accounting conflict.** The one built-in conflict is between built-in accounting and outside accounting — only one can be on. Turn on one while the other is on and it's refused; turn the other off first. Full general-ledger accounting is listed but not available yet; trying to turn it on tells you it's not ready.\n- **Discovery and jumping to the recommendation.** Discovery remembers your place as you answer, so the browser's back button works, and it shows a live leaning recommendation with a confidence level. The final screen lets you jump straight to the recommended preset or pick an alternative; **Skip Discovery** drops you on the Capabilities list to set things up by hand.\n- **Comparing presets.** On the Presets browser, Compare mode lets you pick two to four presets (Custom is left out) and shows them side by side before you commit. Consultant mode shows each feature's underlying code throughout the list, detail pages, and relationships.\n\n## Carrier integrations\n\nExtra fields and lifecycle notes for the Carriers page (available for Admins, Managers, and Office Managers when shipping is turned on).\n\n- **Service ID and carrier code.** When you create a carrier, you can set a service ID (which links the carrier to the shipping connection behind it) and its standard carrier code, alongside the name, code, connection type, tracking-updates setting, and whether a scan is required to ship. The service ID field shows only for Api carriers.\n- **Requires scan to ship.** This is on by default and is enforced on the Shipments page — a package must be scanned before it can be marked shipped for that carrier. It's set per carrier when you create it.\n- **Account number.** The sign-in form takes a client ID, secret, an optional account number, and whether you're testing or live. Some carriers need the account number for rates and labels; the secret is stored securely and never shown back.\n- **Moving from test mode to live.** Set things up in test mode, run the connection test until it passes, then re-save the sign-in details switched to live. Going live is just a re-save of the sign-in details, not a separate step.\n- **Active vs inactive.** Inactive carriers are hidden by default. There's no per-row edit or deactivate button in the table. Whether a carrier is active feeds rate comparison, label creation, and tracking.\n\n## Sales Tax & Currencies\n\nRules, access, and rate-source notes for the tax and currency admin pages.\n\n- **Sales tax rules and access.** Anyone signed in can view the rates; Admins and Managers can create and edit; only Admins can delete. A rate needs a name (up to 100 characters) and a code (up to 20), the posting account is up to 100 characters, and the rate must be between 0 and 100 percent even though it's stored as a fraction behind the scenes. Each code must be unique.\n- **Currency access.** Both the currencies page and the exchange rates are Admin-only. Each currency carries a code, name, symbol, decimal places (the yen uses 0, most use 2), whether it's the base currency, whether it's active, and its display order. The base currency comes from a system setting and is cached briefly.\n- **Converting amounts.** There's a convert tool that takes a from-currency, a to-currency, an amount, and a date. For now, currency conversion is a placeholder: it returns the amount unchanged and treats every rate as 1.0 until a real rate provider is added. Stored rates note where they came from — entered by hand, from a provider, or from a bank — and rates you type in are always marked as entered by hand.\n- **Same-currency conversions.** Converting a currency to itself always uses a rate of 1.0 and returns the amount unchanged, so it's a safe no-op rather than an error.\n- **Filtering the history.** You can filter the rate history by the from-currency, to-currency, and a date range, and it shows the newest first. The history keeps every rate: setting a rate for a pair on a date you already have updates that entry, but a new date adds a fresh entry that takes over from the old one, so past rates stay on record.","sections":[]}"""
        });
