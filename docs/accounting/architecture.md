...
| `INV-001`/`JOB-001`/`AP-001`/`BANK-*` etc. | UI scaffolding present on `feat/accounting-gl-phase1` (forge-ui: trial-balance, P&L, balance-sheet, cash-flow, AR/AP aging, GRNI, bank-rec, period-close). Posting wiring per the roadmap (§4) is in progress. |
...
- UI i18n: the Phase-1 forge-ui components currently render hardcoded strings — externalize to `en.json`/`es.json` before the suite leaves "dark" status.
